...
const BeforeAfterViewer = ({ beforeImg, afterImg, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black bg-opacity-80 flex justify-center items-center p-4">
      <div
        className="relative w-full max-w-4xl h-[300px] md:h-[500px] overflow-hidden rounded-xl shadow-lg bg-black"
        ref={containerRef}
        onMouseMove={handleSliderMove}
        onTouchMove={(e) => handleSliderMove(e.touches[0])}
      >
        {/* AFTER - fills the box (blurred backdrop) + full uncropped photo on top */}
        ...
        {/* BEFORE - same treatment, revealed by the slider via clip-path on the wrapper */}
        <div
        ...
        <div className="absolute top-2 left-2 bg-black bg-opacity-50 text-white px-3 py-1 rounded z-20">Before</div>
        <div className="absolute top-2 right-2 bg-black bg-opacity-50 text-white px-3 py-1 rounded z-20">After</div>
